...
//Lab Test
  ...
  echo ($i <= 1) ? " <tr>\n" : "";
  echo "  <td width='50%' align='center' nowrap>\n";
  echo "   <select name='Lab Test' style='width:96%' onchange='codeselect(this)'>\n";
  echo "    <option value=''> " . xlt('Lab Test') . "\n";
  $tres = sqlStatement("SELECT c.Service_Id,c.code,c.code_text,c.code_type " .
    "FROM codes AS c WHERE " .
    " c.active = 1 AND c.code_type=7  " .
    "ORDER BY c.code");
  while ($trow = sqlFetchArray($tres)) {
    echo "    <option value='Lab Test|" . attr($trow['code']) . '|' . attr($trow['code_text']) . "'>" .
      text($trow['code']);
    //if ($trow['name'] !== $trow['selector']) echo ' ' . text($trow['name']);
    echo "</option>\n";
  }
  ...
  echo "    <option value=''> " . xlt('Services') . "\n";
  $tres = sqlStatement("SELECT c.Service_Id,c.code,c.code_text,c.code_type " .
    "FROM codes AS c WHERE " .
  ...
  $tres = sqlStatement("SELECT c.Service_Id,c.code,c.code_text,c.code_type " .
    ...
    "ORDER BY c.code");
  while ($trow = sqlFetchArray($tres)) {
    echo "    <option value='Doctor Charges|" . attr($trow['code']) . '|' . attr($trow['code_text']) . "'>" .
      text($trow['code_text']);
    //if ($trow['name'] !== $trow['selector']) echo ' ' . text($trow['name']);
    echo "</option>\n";
  }
