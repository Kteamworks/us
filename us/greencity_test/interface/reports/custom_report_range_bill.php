    ...
    foreach($newpatient as $patient){
        ...
        */
        ...
        //print "<h1>".xlt('Patient Data').":</h1>";
               $patdata = getPatientData($pids[$iCounter], 'age,age_days,age_months,rateplan,date,sex,DOB,genericname1,fname,mname,lname,pubpid,street,city,state,postal_code,providerID');
        ...
        if (count($patient) > 0) {
            ...
            //foreach ($patient as $be) {
				...
				foreach ($billing as $b)				
				{
					...
					$counta=sqlStatement("SELECT max(date) d,sum(fee) as st ,code_type, count(code_type) as c from billing where activity='1' and encounter='".$encounter."' and code_type='".$ct."'");
					...
					echo "<td class='text'>".text($b['code_text']) . "</td>";
...
//		   echo "<tr style='border-top: 1px solid #000;'><td colspan=6>&nbsp; </td></tr>";
	...
	echo "</table>";
        ...
        print "<br/><br/>".xlt('Signature').":  _______________________________________________";
...
<form method="post" name="theform" id='theform' action="custom_report_range_bill.php">
