...
if ($_POST['form_submit'] && !empty($_POST['form_line'])) { 
  foreach ($_POST['form_line'] as $lino => $line_value) {
	...
	$act=1;
	$grpn="Default";
	$q=sqlStatement("SELECT provider_id, encounter_id FROM procedure_order WHERE patient_id ='$pid'");
	$res=sqlFetchArray($q);
	$encounter=$res['encounter_id'];
...
while ($row = sqlFetchArray($res)) {
 ...
 $query= "(SELECT $selects FROM procedure_type AS pt2 " .
    ...
    "LEFT JOIN procedure_type AS pt2 ON (case when $pt2cond!=0 then $pt2cond else $pt3cond end)AND $joincond " .
    "WHERE $pscond) " .
