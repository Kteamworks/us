...
$res = sqlStatement("SELECT option_id, title FROM list_options WHERE " .
  "list_id = 'LabSpecific' ORDER BY seq, title");
...
if(isset($_POST['txtHint']))
{

	$patient=$_POST['patientID'];
	$status=$_POST['txtHint'];
	alert($status);
	sqlStatment("Update procedure_order set status='".$status."' where patient_id='".$patient."'");
...
setInterval( function () {
 ...
 //$('#pt_table tbody tr').live('click', function (e) {
  ...
  var orderid=$(this).find('td').eq(3).html();
  //alert(newpid);
...
<?php } ?>
 ...
 $(document).ready(function() {
    $('ul.menu1 li').live("click", function() {
		var name=$(this).text();
		var review=1;
		if(name=="Sample Collection")
		{
		top.restoreSession();
		document.location.href="../../main/finder/specimen_collected.php?set_pid=" + newpid+'&encounter='+enc+'&orderid='+orderid;
		}else
		if(name=="Sample Received")
		{
		top.restoreSession();
		document.location.href="../../main/finder/specimen_received.php?set_pid=" + newpid+'&encounter='+enc+'&orderid='+orderid+'&name='+name;
		}else
			
		if(name=="Add Results")
		{
		top.restoreSession();
		document.location.href="../../orders/orders_results.php?set_pid=" + newpid+'&encounter='+enc+'&review='+review+'&orderid='+orderid;
		}else
		if(name=="Pending")
		{
		//alert(newpid);
