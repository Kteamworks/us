...
$drug_tmp = mysql_fetch_object($drug_tmp1);
	...
	$qry = 'INSERT INTO prescriptions (patient_id,date_added,encounter,drug_id,drug,form,dosage,quantity,size,unit,route,drug_intervals,note) values ("' . $ptid . '","'. date('d-m-y').'","' . $encounter . '","' . $drug_id . '","' .$drugs->name. '","' .$values->dosagetype. '","' .$values->take. '","' .$values->quantity. '","' .$drugs->size. '","' .$values->units. '","' .$drugs->route. '","' .$drug_intervals. '","' .$values->note. '")';
