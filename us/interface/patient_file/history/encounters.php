...
<?php } ?>
  ...
  <th><?php echo htmlspecialchars( xl('Done By'), ENT_NOQUOTES); ?></th>
  <th><?php echo htmlspecialchars( xl('Doctor'), ENT_NOQUOTES);    ?></th>
  <th><?php echo htmlspecialchars( xl('Department'),ENT_NOQUOTES);    ?></th>
  <th><?php echo htmlspecialchars( xl('Visit Type'),ENT_NOQUOTES);    ?></th>
<?php } ?>
...
<?php } else { ?>
  <th colspan='5'><?php echo htmlspecialchars( (($GLOBALS['phone_country_code'] == '1') ? xl('Billing') : xl('Price Code')), ENT_NOQUOTES); ?></th>
<?php } ?>
...
$query = "SELECT fe.*,fe.pc_catid,u.specialty, f.user, u.fname, u.mname, u.lname " . $from .  
...
while ($result4 = sqlFetchArray($res4)) {
       ...
       foreach ($encarr as $enc) {
		...
		echo "<td>".$enc['user'];
            ...
            // see this encounter's notes.
			 ...
			 foreach ($encarr as $enc) {
                ...
                if ($issue) {
                  ...
                  echo htmlspecialchars(xl_form_title($enc['user']), ENT_NOQUOTES);
                  echo "<br>";
                  echo "<div class='encreport' style='padding-left:10px;'>";
                  // Use the form's report.php for display.  Forms with names starting with LBF
                  // are list-based forms sharing a single collection of code.
                  if (substr($formdir,0,3) == 'LBF') {
                    ...
                    //include_once($GLOBALS['incdir'] . "/forms/$formdir/report.php");
					echo htmlspecialchars( ', ' . $result4['fname'] . ' ' . $result4['mname'], ENT_NOQUOTES);
                    //call_user_func($formdir . "_report", $pid, $result4['encounter'], 2, $enc['form_id']);
                  }
                  echo "</div>";
                }
            ...
            $provname = '&nbsp;';
			$dept=$result4['specialty'];
			$visity=$result4['pc_catid'];
			$e=sqlStatement("select pc_catname from openemr_postcalendar_categories where pc_catid='".$visity."'");
			$r=sqlFetchArray($e);
			$visity=$r['pc_catname'];
            /* if($visity==12)
            	{
