...
if (isset($mode)) {
  ...
  if ($mode == "delete") {
    ...
    $crow = sqlQuery("SELECT COUNT(*) AS count FROM codes WHERE " .
	  ...
	  "units = '"    . ffescape($units)    . "' AND " .
	    "provider = '"    . ffescape($provider)    . "' AND " .
...
<?php if ($GLOBALS['concurrent_layout']) {?>
	<span class='title'><?php echo xlt('Create DRG  Budget'); ?></span>
<?php } else { ?>
...
<tr>
<td><b><?php echo xlt('DRG Code'); ?>:</b> 470
<a id="myID"  class="editscript" data-toggle="modal" data-target="#myModal">&nbsp;&nbsp;<span><i class="fa fa-pencil-square-o" data-toggle="tooltip" data-placement="top" title="Edit DRG " aria-hidden="true"></i></span></a>
     <!-- Modal -->
<?php
$qry=sqlStatement("SELECT * from code_types where ct_key='DRG470'");
$qry1=sqlFetchArray($qry);
?>
  <div class="modal fade" id="myModal" role="dialog">
    <div class="modal-dialog">
      ...
      <div class="modal-content">
 	  <form id='merge-form' method="post" action="edit_super_bill.php"  >
		...
		<input type="text" name="drgdescription" id="drgdescription" class='form-control' value='<?php echo attr($qry1['drgdescription']) ?>'>
		</input>
		</div>
				<div class="form-group">
		<label>CMS Payout</label>
		<input type="text" name="cmspayout" id="cmspayout" class='form-control' value='<?php echo attr($qry1['cmspayout']) ?>'>
		</input>
			</div>
				<div class="form-group">
		<label>Additional Pay Conditions:</label>
<br>
		<label>Alcohol</label>
		<input type="text" name="alcohol" id="alcohol" class='form-control' value='<?php echo attr($qry1['alcohol']) ?>'>
		</input>
			</div>
				<div class="form-group">
		<label>Tobacco</label>
		<input type="text" name="tobacco" id="tobacco" class='form-control' value='<?php echo attr($qry1['tobacco']) ?>'>
		</input>
			</div>
				<div class="form-group">
		<label>Obesity</label>
		<input type="text" name="obesity" id="obesity" class='form-control' value='<?php echo attr($qry1['obesity']) ?>'>
		</input>
				</div>
				<div class="form-group">
		<label>Dementia</label>
		<input type="text" name="dementia" id="dementia" class='form-control' value='<?php echo attr($qry1['dementia']) ?>'>
		</input>
			</div>
				<div class="form-group">
		<label>Vision Impairment</label>
		<input type="text" name="vision_impairment" id="vision_impairment" class='form-control' value='<?php echo attr($qry1['vision_impairment']) ?>'>
		</input>
	</div>
			
        </div>
        <div class="modal-footer">
		<button type="button" id="submit-merge" class="btn btn-primary" >Submit</button>
 ...
 $("#submit-merge").on("click", function(){
var alcohol = $("#alcohol").val();
var drgdescription = $("#drgdescription").val();
var obesity = $("#obesity").val();
var vision_impairment = $("#vision_impairment").val();
var dementia = $("#dementia").val();
var tobacco = $("#tobacco").val();
var cmspayout = $("#cmspayout").val();
var dataString = 'alcohol='+ alcohol + '&drgdescription='+ drgdescription + '&tobacco='+ tobacco + '&obesity='+ obesity + '&dementia='+ dementia + '&vision_impairment='+ vision_impairment+ '&cmspayout='+ cmspayout ;
    $.ajax({
    type: "POST",
            url: 'edit_super_bill.php',
            dataType: "html",
            data: dataString,
...
<tr>
<td><b><?php echo xlt('DRG Description'); ?>:</b> <?php echo attr($qry1['drgdescription']) ?></td>
</tr>
<tr>
<td><b><?php echo xlt('CMS Payout'); ?>:</b> $<?php echo attr($qry1['cmspayout']) ?></td>
</tr>
<tr>
<td><b><?php echo xlt(' Additional pay conditions'); ?>:</b></td>
</tr>
<tr>
<td style='font-size:13px'><?php echo xlt('Alcohol'); ?>: $<?php echo attr($qry1['alcohol']) ?></td>
</tr>
<tr>
<td style='font-size:13px'><?php echo xlt('Tobacco'); ?>: $<?php echo attr($qry1['tobacco']) ?></td>
</tr><tr>
<td style='font-size:13px'><?php echo xlt('Obesity'); ?>:$<?php echo attr($qry1['obesity']) ?></td>
</tr><tr>
<td style='font-size:13px'><?php echo xlt('Dementia'); ?>: $<?php echo attr($qry1['dementia']) ?></td>
</tr><tr>
<td style='font-size:13px'><?php echo xlt('Vision Impairment'); ?>: $<?php echo attr($qry1['vision_impairment']) ?></td>
</tr>
</table>
<center>
 <table border='0' cellpadding='0' cellspacing='0'>
 ...
 <tr>
 <!--<td  style="display:none;"><?php echo xlt('Type'); ?>:</td>
  <td width="5">
  </td>
  <td> -->

   <?php if ($mode != "modify") { ?>
     <select name="code_type" onchange='doc(this.value)'  style="display:none;">
   <?php } ?>
...
<?php
$ores = sqlStatement("SELECT code from codes where code_type=6");
while ($orow = sqlFetchArray($ores)) {
  echo "    <option value='" . $orow['code'] . "'";
  if ($orow['code'] == $code) echo " selected";
  echo ">" . $orow['code'] . "</option>\n";
}
?>
   </select>
...
<?php } ?>
 ...
 <tr>
  ...
  <td>
   ...
   <?php if ($mode == "modify") { ?>
     <input type='text' size='3' name="units" readonly="readonly" value='<?php echo attr($units) ?>'>
   <?php } else { ?>
     <input type='text' size='3' name="units" value='<?php echo attr($units) ?>'>
   <?php } ?>
   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
   <?php echo xlt('Provider'); ?>:
   <?php if ($mode == "modify") { ?>
     <input type='text' size='6' name='provider'  readonly='readonly' value='<?php echo attr($provider) ?>' />
   <?php } else { ?>
    <select name='provider' id='provider' >
<?php
$ores = sqlStatement("SELECT  title from list_options where list_id='Provider'");
while ($orow = sqlFetchArray($ores)) {
  echo "    <option value='" . $orow['title'] . "'";
  if ($orow['title'] == $provider) echo " selected";
...
<table border='0' cellpadding='5' cellspacing='0' width='96%'>
 ...
 <td><span class='bold'><?php echo xlt('Type'); ?></span></td>
  <td><span class='bold'><?php echo xlt('Service'); ?></span></td>
   <td align='right' nowrap><span class='bold'><?php echo xlt('Units'); ?></span></td>
...
?>
<td align='right' nowrap><span class='bold'><?php echo xlt('Total Cost'); ?></span></td>
<td align='right' nowrap><span class='bold'><?php echo xlt('Provider'); ?></span></td>
  <td></td>
  <td></td>
...
if (!empty($all)) {
  ...
  foreach($all as $iter) {
	...
	echo "  <td class='text' align='right'>" . text($iter['units']) . "</td>\n";
   ...
   // }
    ...
    while ($prow = sqlFetchArray($pres)) {
      echo "<td class='text' align='right'>" . text(bucks($prow['pr_price'])) . "</td>\n";
	   $total=$prow['pr_price']*$iter['units'];
	echo "  <td class='text' align='right'>" . text(bucks($total)) . "</td>\n";
	$grandtotal+=$total;
    }
	
 echo "  <td class='text' align='right'>" . text($iter['provider']) . "</td>\n";
    if ($iter["code_external"] > 0) {
...
echo "<td>" ;
echo "</td>";
 echo "<td align='right'>" .xlt('Grand Total') . ":</td>\n";
/* $total=sqlStatement("SELECT sum(b.pr_price*units) price
from codes a, prices b
where a.id=b.pr_id
and a.code_type=6 and b.pr_level='Standard'");
$total1=sqlFetchArray($total);
 */  
 echo "<td align='right'><b>" . text(bucks($grandtotal)) . "</td></b>\n";
