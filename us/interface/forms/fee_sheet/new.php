...
$gbfa=sqlFetchArray($grpbillq); */
  ...
  if (! $code_text) {
    $sqlArray = array();
    $query = "select id,code_type as ct,service_id, units, code_text from codes where code_type = ? " .
      " and " .
      "code = ? and ";
  ...
  if ($id) {
   ...
   echo "<input type='hidden' name='bill[".attr($lino)."][ct]' value='".attr($ct)."'>";
   echo "<input type='hidden' name='bill[".attr($lino)."][service_id]' value='".attr($service_id)."'>";
  ...
  if ($billed) {
   ...
   /* echo "  <td class='billcell' align='center'$usbillstyle><input type='checkbox'" .
    ...
    echo "  <td class='billcell' align='center'><input type='checkbox'" .
	  ...
	  if($grpb==1)
	  {
	  echo "  <td class='billcell' align='center'><input type='checkbox' disabled name='grpbill[]' " .
  ...
  // not billed
   ...
   /* if ($code_types[$codetype]['claim'] && !$code_types[$codetype]['diag']) {
      echo "  <td class='billcell' align='center'$usbillstyle><input type='text' name='bill[".attr($lino)."][notecodes]' " .
        "value='" . htmlspecialchars($notecodes, ENT_QUOTES) . "' size='8' /></td>\n";
    }
    else {
      echo "  <td class='billcell' align='center'$usbillstyle></td>\n";
    }
...
function genProviderSelect($selname, $toptext, $default=0, $disabled=false) {
  $query = "SELECT id, fname, lname FROM users WHERE " .
...
$bilgrp = $_POST['grpbill'];
$bilgrpval=$_POST[''];
  if(empty($bilgrp)) 
  {
    echo("You didn't select any grp.");
  } 
  else
  {
    $N = count($bilgrp);
    //sqlStatement("TRUNCATE table bill_encounter");
   // echo("You selected $N (s):'".$bilgrp[$i]."'");
	//sqlStatement("UPDATE billing set grpbill='0' where encounter='".$encounter."'");
    for($i=0; $i < $N; $i++)
    {
      //echo($bilgrp[$i] . " ");
	   //sqlStatement("TRUNCATE table bill_encounter");
	  $grpbillid=$bilgrp[$i];
	  sqlStatement("UPDATE billing set grpbill='1' where id='".$grpbillid."'");
    }
    ...
    if ($id) {
      if ($del) {
		...
		sqlQuery("Update billing_main_copy set total_charges=total_charges - ? where encounter=?",array($fee,$encounter));   
		  ...
		  $a=sqlStatement("select fee from billing where id=?",array($id));
		  $d=sqlFetchArray($a);
		  $feeold=$d['fee'];
...
<span class="title"><?php echo xlt('Create New DRG'); ?></span><br><br>

<input type='hidden' name='newcodes' value=''>
<table>
<tr>
	<td align="left" class="forms" class="forms"><b><?php echo xlt('DRG Code' ); ?>:</td>
		<td class="forms">
			<label class="forms-data"> <?php echo DRG470; ?></label>
				
		</td>
		</tr>
		<tr>
	<td align="left" class="forms" class="forms"><b><?php echo xlt('DRG Description' ); ?>:</td>
		<td class="forms">
			<label class="forms-data"> <?php echo KneeReplacementSurgery; ?></label>
				
		</td>
		</tr>
		<tr>
	<td align="left" class="forms" class="forms"><b><?php echo xlt('CMS Payout' ); ?>:</td>
		<td class="forms">
			<label class="forms-data"> <?php echo 25000; ?></label>
				
		</td>
		</tr>
		<tr>
	<td align="left" class="forms" class="forms"><b><?php echo xlt('Additional Pay Conditions' ); ?>:</td>
		
		</tr>
		<tr>
	<td align="left" class="forms" class="forms"><b><?php echo xlt('Alcohol' ); ?>:</td>
		<td class="forms">
			<label class="forms-data"> <?php echo 1000; ?></label>
				
		</td>
		</tr><tr>
	<td align="left" class="forms" class="forms"><b><?php echo xlt('Tobacco' ); ?>:</td>
		<td class="forms">
			<label class="forms-data"> <?php echo 1000; ?></label>
				
		</td>
		</tr><tr>
	<td align="left" class="forms" class="forms"><b><?php echo xlt('Obesity' ); ?>:</td>
		<td class="forms">
			<label class="forms-data"> <?php echo 1000; ?></label>
				
		</td>
		</tr><tr>
	<td align="left" class="forms" class="forms"><b><?php echo xlt('Dementia' ); ?>:</td>
		<td class="forms">
			<label class="forms-data"> <?php echo 1000; ?></label>
				
		</td>
		</tr><tr>
	<td align="left" class="forms" class="forms"><b><?php echo xlt('Vision Impairment' ); ?>:</td>
		<td class="forms">
			<label class="forms-data"> <?php echo 1000; ?></label>
...
// Create one more drop-list, for Products.
  ...
  echo ($i <= 1) ? " <tr>\n" : "";
  echo "  <td width='50%' align='center' nowrap>\n";
  echo "   <select name='Services' style='width:96%' onchange='codeselect(this)'>\n";
  echo "    <option value=''> " . xlt('DRG 470') . "\n";
  $tres = sqlStatement("SELECT c.Service_Id,c.code,c.code_text,c.code_type " .
    "FROM codes AS c WHERE " .
    " c.active = 1 AND c.code_type=6  " .
    "ORDER BY c.code");
  while ($trow = sqlFetchArray($tres)) {
    echo "    <option value='Services|" . attr($trow['code']) . '|' . attr($trow['code_text']) . "'>" .
      text($trow['code']);
...
<table cellspacing='5'>
 <tr>
  ...
  <td class='billcell'><b><?php echo xlt('Service');?></b></td>
<?php if (modifiers_are_used(true)) { ?>
  <td class='billcell'><b><?php echo xlt('Modifiers');?></b></td>
<?php } ?>
<?php if (fees_are_used()) { ?>
  <td class='billcell' align='right'><b><?php echo xlt('Unit Cost ');?></b>&nbsp;</td>
