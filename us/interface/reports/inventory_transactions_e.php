...
<style type="text/css">
 ...
 body       { font-family:sans-serif; font-size:5pt; font-weight:normal }
 .dehead    { color:#000000; font-family:sans-serif; font-size:5pt; font-weight:bold }
 .detail    { color:#000000; font-family:sans-serif; font-size:5pt; font-weight:normal }
</style>
...
<form method='post' action='inventory_transactions_e.php'>
...
<table border='0' cellpadding='1' cellspacing='2' width='98%'>
 ...
 $message='<div id="report_results"><table border="0" width="32.3%"><tr bgcolor="#dddddd"> <td class="dehead">' . htmlspecialchars(xl('Date'), ENT_NOQUOTES). 
         ' </td><td class="dehead">'.         
		htmlspecialchars(xl('Product'), ENT_NOQUOTES). 
		' </td><td class="dehead">'.		
		htmlspecialchars(xl('Who'), ENT_NOQUOTES). 
		'</td> <td class="dehead" align="right">'.
		htmlspecialchars(xl('Qty'), ENT_NOQUOTES). 
		'</td><td class="dehead" align="right">'.
		htmlspecialchars(xl('Amount'), ENT_NOQUOTES). 
		'</td>'.
 ...
 /*
    For pure prescriptions
     select     
                ...
                ,r.id as id, r.date_modified AS prescriptions_date_modified, r.dosage as dosage, r.route as route, r.interval as hinterval, r.refills as refills, r.drug as drug, 
		            r.form as hform, r.size as size, r.unit as hunit, d.name as name, d.ndc_number as ndc_number,r.quantity as quantity
    
             from patient_data as pd left outer join users as u on u.id = pd.providerid
             left outer join facility as f on f.id = u.facility_id
             left outer join prescriptions AS r on r.patient_id=pd.pid
...
$res = sqlStatement($query, array($from_date, $to_date));
  while ($row = sqlFetchArray($res)) {
    ...
    $message.='<tr bgcolor="#ddddff"><td class="detail">'. htmlspecialchars(oeFormatShortDate($row['sale_date'])).'</td>'.'<td class="detail">'.htmlspecialchars($row['name']).'</td><td class="detail">'. htmlspecialchars($dpname).'</td> <td class="detail" align="right">'.		  htmlspecialchars($row['quantity']).'</td><td class="detail" align="right">'.htmlspecialchars(bucks(0-$row['fee'])); 
		  '</td></tr>';
...
$pdf->SetAuthor('CJ');
$pdf->SetTitle('TCPDF EMAIL');
$pdf->SetSubject('EMAIL');
...
$pdf->Output($_SERVER['DOCUMENT_ROOT'] . '/output.pdf', 'F');
    ...
    $mail->AddReplyTo($email_sender, $email_sender);
    $mail->SetFrom($email_sender, $email_sender);
    $mail->AddAddress("kavaiidev01@gmail.com", $pt_name);
    $mail->Subject = $email_subject;
    $mail->MsgHTML("<html><body><div class='wrapper'>".$message."</div></body></html>");
