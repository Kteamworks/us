 ...
 */
 ...
 $primary_docs = array(
  ...
  'neo' => array(xl('New Pt')    , 0, 'new/newopd.php'),
...
?>
 ...
 //It is called when a new patient is create/selected from the search menu.
  ...
  str+='<option value=""><?php echo htmlspecialchars( xl('Episode History'), ENT_QUOTES) ?></option>';
  //str+='<option value="New Obs Visit"><?php echo htmlspecialchars( xl('New Obs Visit'), ENT_QUOTES) ?></option>';
  str+='<option value="New Encounter"><?php echo htmlspecialchars( xl('New Episode'), ENT_QUOTES) ?></option>';
  str+='<option value="Past Encounter List"><?php echo htmlspecialchars( xl('Past Episode List'), ENT_QUOTES) ?></option>';
  ...
  <li><a class="collapsed" id="patimg" ><span><?php xl('Patient','e') ?></span></a>
	...
	<?php if($newcrop_user_role['newcrop_user_role']!='erxdoctor'||$_SESSION['authUser']=='DR. NIRMALA.B.M') {?>
	 ...
	 <!-- <?php if($newcrop_user_role['newcrop_user_role']!='erxdoctor') {?>
	  ...
	  <?php }?> -->
      <?php genTreeLink('RTop','new',($GLOBALS['full_new_patient_form'] ? xl('Add Patient') : xl('New'))); ?>
...
<?php } ?>
	...
	if($newcrop_user_role['newcrop_user_role']=='erxadmin'){ ?>
		...
		<?php genMiscLink('RTop','rop','0',xl('Change Voucher Date'), 'reports/voucher_date_change.php'); ?>
	     <?php genMiscLink('RTop','rop','0',xl('Bill Cancel'), 'reports/bill_cancel.php'); ?>
		  <?php if (! $GLOBALS['simplified_demographics']) genTreeLink('RTop','bil',xl('Re-Open the Bill')); ?>
  ...
  <?php // TajEmo Work by CB 2012/06/21 10:41:15 AM hides fees if disabled in globals ?>
  <?php if(!isset($GLOBALS['enable_fees_in_left_menu']) || $GLOBALS['enable_fees_in_left_menu'] == 1){ ?>
  <?php if($_SESSION['authUser']!='usdemo') {?>
  <li><a class="collapsed" id="feeimg" ><span><?php xl('DRG Progress','e') ?></span></a>
    <ul>
      <?php genMiscLink('RBot','cod','0',xl('DRG Progress'),'patient_file/encounter/load_form.php?formname=fee_sheet'); ?>
...
<?php genTreeLink('RTop','orc',xl('Load Compendium')); ?> -
  ...
  <li><a class="collapsed" id="admimg" ><span><?php xl('DRG BUDGET','e') ?></span></a>
    <ul>
	<?php if($_SESSION['authUser']!='usdemo') {?>
      <?php if (acl_check('admin', 'super'    )) genMiscLink('RTop','adm','0',xl('Globals'),'super/edit_globals.php'); ?>
      <?php if (acl_check('admin', 'users'    )) genMiscLink('RTop','adm','0',xl('Facilities'),'usergroup/facilities.php'); ?>
	...
	<?php }?>
      <?php if (acl_check('admin', 'superbill')) genTreeLink('RTop','sup',xl('Create DRG Budget')); ?>
	  <?php if($_SESSION['authUser']!='usdemo') {?>
      <?php if (acl_check('admin', 'super'    )) genMiscLink('RTop','adm','0',xl('Layouts'),'super/edit_layout.php'); ?>
      <?php if (acl_check('admin', 'super'    )) genMiscLink('RTop','adm','0',xl('Lists'),'super/edit_list.php'); ?>
      <?php if (acl_check('admin', 'acl'      )) genMiscLink('RTop','adm','0',xl('ACL'),'usergroup/adminacl.php'); ?>
...
<?php 
  ...
  ?>
      <li><a class="collapsed_lv2"><span><?php xl('Episode Progress','e') ?></span></a>
        <ul>
...
<?php if(acl_check('admin','super')){
	...
	?>
  <li><a class="collapsed_lv2" id="analyticsimg" ><span><?php xl('Episode Profitability','e') ?></span></a>
...
<?php } // end ippf-specific ?>
      <li><a class="collapsed_lv2"><span><?php xl('What-If scenario','e') ?></span></a>
        <ul>
